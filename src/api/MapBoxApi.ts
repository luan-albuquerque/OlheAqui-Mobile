...
export const SearchLocalByLongAndLatMapBox = (LONGITUDE: string, LATITUDE: string) =>
  fetch(
    `https://api.mapbox.com/geocoding/v5/mapbox.places/${LONGITUDE},${LATITUDE}.json?${ACCESS_TOKEN_MAP_BOX}`
  ).then(response => response.json()).then(data => data);